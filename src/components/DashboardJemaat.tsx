...
export default function DashboardJemaat({
  ...
  // Donation Form Controlled States
  const [donationDonorName, setDonationDonorName] = useState(currentUser.name);
  const [donationAmount, setDonationAmount] = useState('');
  const [donationType, setDonationType] = useState('Persepuluhan');
  const [donationNote, setDonationNote] = useState('');
  const [donationPaymentMethod, setDonationPaymentMethod] = useState('Transfer Bank');
  ...
  const [hpBarNotifEnabled, setHpBarNotifEnabled] = useState<boolean>(() => {
    if (typeof window !== 'undefined') {
      return localStorage.getItem('hp_bar_notif_enabled') === 'true' || (typeof Notification !== 'undefined' && Notification.permission === 'granted');
    }
    return false;
  });

  const handleEnableHpBarNotification = async () => {
    if (typeof window !== 'undefined') {
      localStorage.setItem('hp_bar_notif_enabled', 'true');
      setHpBarNotifEnabled(true);
      ...
      if ('Notification' in window) {
        try {
          ...
          console.warn('Native notification request error (handled gracefully):', e);
        }
      }

      showToast('✓ Status Bar HP Notifikasi Aktif! Anda akan menerima pemberitahuan langsung di bilah status ponsel.', 'success');

      if ('serviceWorker' in navigator && navigator.serviceWorker.controller) {
        navigator.serviceWorker.ready.then((reg) => {
          reg.showNotification('SYSTEM MANAJEMEN CHURCH', {
            body: '🔔 Bar HP Aktif! Anda telah terhubung dengan pemberitahuan resmi gereja.',
  ...
  const handleAddComment = (targetId: string, targetType: 'news' | 'devotion', targetTitle: string) => {
    ...
    const initialStatus = isAdminRole ? 'approved' : 'pending';
    ...
    if (isAdminRole) {
      showToast('Komentar Anda berhasil ditambahkan dan dipublikasikan.', 'success');
    } else {
      showToast('Komentar Anda telah terkirim! Komentar akan ditampilkan publik setelah disetujui Admin pada menu Moderasi Komentar.', 'info');
    }
  };
      ...
      {activeTab === 'beranda' && (
        <div className="space-y-8 animate-fade-in bg-[#070B14] p-6 -mx-6 md:-mx-8 -my-6 md:-my-8 min-h-screen text-slate-100">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
            ...
            <div className="bg-[#0F172A]/90 border border-slate-800/80 p-5 rounded-3xl relative overflow-hidden flex flex-col justify-between min-h-[140px] group hover:scale-[1.02] transition-all shadow-lg">
              <div className="flex items-center justify-between">
                <p className="text-slate-400 font-extrabold text-[10px] uppercase tracking-widest">Kas & Donasi Terverifikasi</p>
                <div className="p-2 bg-emerald-500/10 text-emerald-400 rounded-xl border border-emerald-500/20">
                  <Coins className="w-4 h-4" />
                </div>
              </div>
              <p className="text-2xl md:text-3xl font-black text-emerald-400 mt-2">
                Rp {donations.filter((d) => d.status === 'verified').reduce((sum, d) => sum + (d.amount || 0), 0).toLocaleString('id-ID')}
              </p>
            ...
            <div className="bg-gradient-to-r from-slate-900 via-amber-950/40 to-slate-900 border border-amber-500/30 p-6 rounded-3xl space-y-4 shadow-xl relative overflow-hidden">
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  {hpBarNotifEnabled ? (
                    <div className="flex items-center gap-2">
                      <span className="px-3 py-1.5 bg-emerald-500/20 text-emerald-300 font-bold text-[10px] rounded-xl border border-emerald-500/30 flex items-center gap-1.5 shadow-sm">
                        <Check className="w-3.5 h-3.5 text-emerald-400" />
                        <span>Bar HP: AKTIF</span>
                      </span>
                      <button
                        onClick={handleEnableHpBarNotification}
                        className="px-2.5 py-1.5 bg-slate-800 hover:bg-slate-700 text-amber-300 text-[10px] font-bold rounded-xl border border-amber-500/30 transition-all cursor-pointer shadow-sm"
                        title="Tes Notifikasi Bar HP"
                      >
                        Tes Notif
                      </button>
                    </div>
                    ...
                    <button
                      onClick={handleEnableHpBarNotification}
                      className="px-3.5 py-2 bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-400 hover:to-amber-500 text-slate-950 font-black text-xs rounded-xl transition-all cursor-pointer shadow-lg hover:shadow-amber-500/25 flex items-center gap-1.5 active:scale-95"
                    >
          ...
          <div className="bg-white dark:bg-slate-900/60 p-6 rounded-3xl border border-gray-100 dark:border-slate-800 shadow-sm space-y-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              ...
              <div className="space-y-6">
                ...
                {/* Real Donation Confirmation Form */}
                <div className="bg-gray-50/50 dark:bg-slate-950/20 p-5 rounded-2xl border border-gray-100 dark:border-slate-800 space-y-3">
                  ...
                  <form
                    onSubmit={(e) => {
                      ...
                      const newDon: Donation = {
                        id: `don_${Date.now()}`,
                        donorName: donationDonorName.trim() || currentUser.name,
                        userId: currentUser.id,
                        userEmail: currentUser.email,
                      ...
                      showToast(`✓ Laporan donasi Rp ${Number(donationAmount).toLocaleString('id-ID')} berhasil dikirim! Menunggu verifikasi bendahara gereja.`, 'success');
                      setDonationAmount('');
                      setDonationNote('');
                      loadAllData();
                    }}
                  ...
                  >
                    <div>
                      <label className="block text-[9px] font-semibold text-gray-500 uppercase mb-0.5">Nama Donatur</label>
                      <input
                        type="text"
                        required
                        value={donationDonorName}
                        onChange={(e) => setDonationDonorName(e.target.value)}
                        className="w-full p-2 border border-gray-200 dark:border-slate-800 rounded-xl text-xs bg-white dark:bg-slate-900"
                      />
                    </div>
                    <div className="grid grid-cols-2 gap-2">
                      <div>
                        <label className="block text-[9px] font-semibold text-gray-500 uppercase mb-0.5">Jumlah Donasi (Rp)</label>
                        <input
                          type="number"
                          required
                          ...
                          className="w-full p-2 border border-gray-200 dark:border-slate-800 rounded-xl text-xs font-bold font-mono bg-white dark:bg-slate-900"
                        />
                      </div>
                      <div>
                        <label className="block text-[9px] font-semibold text-gray-500 uppercase mb-0.5">Jenis Persembahan</label>
                        <select
                          value={donationType}
                    ...
                    <div>
                      <label className="block text-[9px] font-semibold text-gray-500 uppercase mb-0.5">Metode Pengiriman</label>
                      <select
                    ...
                    <div>
                      <label className="block text-[9px] font-semibold text-gray-500 uppercase mb-0.5">Catatan / Doa Penyerta</label>
                      <textarea
                        value={donationNote}
                        onChange={(e) => setDonationNote(e.target.value)}
                        placeholder="Tuliskan pesan atau doa penyerta..."
                        className="w-full h-16 p-2 border border-gray-200 dark:border-slate-800 rounded-xl text-xs bg-white dark:bg-slate-900 resize-none"
                      />
                    </div>
                    <button type="submit" className="w-full py-2.5 bg-brand text-white font-bold text-xs rounded-xl shadow-sm cursor-pointer hover:opacity-90 transition-opacity">
                      Kirim Laporan Konfirmasi Kas
                    </button>
                  </form>
                </div>
              </div>
            ...
            {/* Riwayat Donasi & Laporan Kas */}
            <div className="pt-6 border-t border-gray-100 dark:border-slate-800 space-y-4">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                <div>
                  <h3 className="font-display font-bold text-sm uppercase text-gray-800 dark:text-gray-100">Riwayat Laporan & Persembahan Kas Anda</h3>
                  <p className="text-xs text-gray-400">Daftar transaksi donasi yang Anda kirimkan beserta status validasi resmi sekretariat gereja.</p>
                </div>
              </div>
              ...
              <div className="bg-white dark:bg-slate-900 border border-gray-100 dark:border-slate-800 rounded-2xl overflow-hidden shadow-sm">
                <div className="overflow-x-auto">
                  <table className="w-full text-left text-xs">
                    <thead className="bg-gray-50 dark:bg-slate-950/50 text-gray-500 uppercase text-[9px] font-black border-b border-gray-100 dark:border-slate-800">
                      <tr>
                        <th className="p-3">Tanggal</th>
                        <th className="p-3">Jenis Persembahan</th>
                    ...
                    <tbody className="divide-y divide-gray-100 dark:divide-slate-800">
                      {donations
                        .filter((d) => d.userId === currentUser.id || d.userEmail === currentUser.email)
                        .map((item) => (
                          <tr key={item.id} className="hover:bg-gray-50/50 dark:hover:bg-slate-800/40">
                            ...
                            <td className="p-3">
                              {item.status === 'verified' && (
                                <span className="inline-flex items-center gap-1 bg-emerald-50 text-emerald-700 dark:bg-emerald-950/80 dark:text-emerald-400 px-2 py-0.5 rounded-full text-[9px] font-black border border-emerald-200/80 dark:border-emerald-800">
                                  <CheckCircle2 className="w-3 h-3 text-emerald-600" /> Terverifikasi Bendahara
                                </span>
                              )}
                              {item.status === 'pending' && (
                                <span className="inline-flex items-center gap-1 bg-amber-50 text-amber-700 dark:bg-amber-950/80 dark:text-amber-400 px-2 py-0.5 rounded-full text-[9px] font-black border border-amber-200/80 dark:border-amber-800">
                                  ⏳ Menunggu Audit
                                </span>
                              )}
                              {item.status === 'rejected' && (
                                <span className="inline-flex items-center gap-1 bg-rose-50 text-rose-700 dark:bg-rose-950/80 dark:text-rose-400 px-2 py-0.5 rounded-full text-[9px] font-black border border-rose-200/80 dark:border-rose-800">
                                  ✗ Laporan Ditolak
                                </span>
                              )}
                            </td>
                  ...
                  {donations.filter((d) => d.userId === currentUser.id || d.userEmail === currentUser.email).length === 0 && (
                    <p className="text-center py-8 text-gray-400 text-xs">Anda belum pernah mengonfirmasi laporan donasi. Gunakan formulir di atas untuk mengirimkan konfirmasi kas.</p>
                  )}
                </div>
              </div>
            </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {schedules.map((sch) => {
                  ...
                  return (
                    <div
                      key={sch.id}
                      className="p-5 bg-white border border-gray-200/80 rounded-2xl space-y-3 hover:border-brand transition-all shadow-sm flex flex-col justify-between group relative"
                    >
                      ...
                      <div className="flex items-center gap-2 pt-2">
                        <button
                          ...
                          className={`flex-1 py-2.5 font-bold text-xs rounded-xl transition-all cursor-pointer flex items-center justify-center gap-1 ${
                            isReminded
