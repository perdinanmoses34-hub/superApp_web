...
export default function AdminModules({
  ...
  const [manualDonationForm, setManualDonationForm] = useState({
    ...
    amount: '',
    type: 'Persepuluhan',
    paymentMethod: 'Tunai / Offline',
    note: '',
  });

  const [isEditingBankSettings, setIsEditingBankSettings] = useState(false);
  const [bankSettingsForm, setBankSettingsForm] = useState({
    bankName: settings.bankName || 'BCA (Bank Central Asia)',
    bankAccountNo: settings.bankAccountNo || '8290123456',
    bankAccountName: settings.bankAccountName || 'SYSTEM MANAGEMENT CHURCH (CMS)',
    qrisUrl: settings.qrisUrl || 'https://images.unsplash.com/photo-1618005182384-a83a8bd57fbe?w=500&auto=format&fit=crop&q=80',
  });
  ...
  // Donation Filters & Handlers
  const [donationFilterStatus, setDonationFilterStatus] = useState<'all' | 'pending' | 'verified' | 'rejected'>('all');
  const [donationFilterType, setDonationFilterType] = useState<string>('Semua');
  ...
  const handleSaveBankSettings = (e: React.FormEvent) => {
    ...
    const updatedSettings: ChurchSettings = {
      ...settings,
      bankName: bankSettingsForm.bankName,
      bankAccountNo: bankSettingsForm.bankAccountNo,
      bankAccountName: bankSettingsForm.bankAccountName,
      qrisUrl: bankSettingsForm.qrisUrl,
    };
    MockDatabase.saveSettings(updatedSettings, currentUser);
    if (onSettingsSaved) {
      onSettingsSaved(updatedSettings);
    }
    setIsEditingBankSettings(false);
    showToast('✓ Pengaturan Rekening Bank & QRIS berhasil diperbarui!', 'success');
  };

  const handleSaveManualDonation = (e: React.FormEvent) => {
    e.preventDefault();
    if (!manualDonationForm.amount || Number(manualDonationForm.amount) <= 0) {
      showToast('Jumlah donasi harus lebih dari 0', 'error');
      return;
    }
    const newDon: Donation = {
      id: `don_${Date.now()}`,
      donorName: manualDonationForm.donorName.trim() || 'Jemaat (Kas Tunai)',
      amount: Number(manualDonationForm.amount),
      type: manualDonationForm.type || 'Persepuluhan',
      paymentMethod: manualDonationForm.paymentMethod || 'Tunai / Offline',
      note: manualDonationForm.note.trim() || 'Pencatatan kas fisik ibadah',
      date: new Date().toISOString(),
      status: 'verified',
      verifiedBy: currentUser.name,
      verifiedDate: new Date().toISOString(),
    };
    MockDatabase.addDonation(newDon);
    showToast('✓ Donasi/Kas fisik berhasil dicatat & langsung terverifikasi!', 'success');
    setIsAddingManualDonation(false);
    setManualDonationForm({ donorName: '', amount: '', type: 'Persepuluhan', paymentMethod: 'Tunai / Offline', note: '' });
    loadAllData();
  };

  const handleVerifyDonation = (id: string, status: 'verified' | 'rejected') => {
    MockDatabase.verifyDonation(id, status, currentUser.name, currentUser);
    loadAllData();
    showToast(
      status === 'verified'
        ? '✓ Status donasi DIVERIFIKASI & langsung tercatat pada Laporan Kas!'
        : 'Status donasi diubah menjadi DITOLAK.',
      status === 'verified' ? 'success' : 'info'
    );
  };

  const handleExportDonationsCSV = () => {
    const headers = ['ID', 'Tanggal', 'Nama Donatur', 'Email', 'Jumlah (Rp)', 'Jenis Persembahan', 'Metode', 'Status', 'Catatan', 'Verifikator'];
    ...
    link.setAttribute('download', `Laporan_Kas_Donasi_${new Date().toISOString().slice(0, 10)}.csv`);
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    showToast('✓ Laporan Kas & Donasi berhasil diunduh dalam format CSV!', 'success');
  };
        ...
        <div className="animate-fade-in max-w-xl mx-auto bg-gray-50/50 border border-gray-100 p-6 rounded-2xl space-y-4">
          ...
          {activeTab === 'admin_events' && (
            <form onSubmit={handleSaveEvent} className="space-y-3">
              ...
              <button type="submit" className="w-full py-2.5 bg-brand text-white font-bold text-xs rounded-xl shadow-md cursor-pointer hover:bg-brand-dark transition-colors">
                <Check className="w-4 h-4 inline mr-1" /> Simpan Event
              </button>
            </form>
          )}

          {activeTab === 'admin_schedules' && (
            <form onSubmit={handleSaveSchedule} className="space-y-4 bg-slate-50/50 p-5 rounded-2xl border border-slate-200/80 mb-6">
              <div className="flex items-center justify-between border-b border-slate-200/60 pb-3 mb-2">
                <h3 className="font-bold text-xs text-slate-800 uppercase tracking-wide flex items-center gap-1.5">
                  ...
                  {isCreatingNew ? 'Form Tambah Jadwal Ibadah Baru' : 'Form Edit Jadwal Ibadah'}
                </h3>
                <span className="text-[10px] text-slate-400 font-medium">Informasi ini akan ditampilkan di Dashboard Jemaat</span>
              </div>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-semibold text-gray-700 mb-1">Nama Sesi / Label Sesi</label>
                  <input
                    type="text"
                    required
                    value={editingItem.sessionName || ''}
                    onChange={(e) => setEditingItem({ ...editingItem, sessionName: e.target.value })}
                    placeholder="Contoh: Sesi I (Pagi) atau Doa Malam"
                    className="w-full p-2.5 bg-white border border-gray-200 rounded-xl text-xs font-medium focus:ring-2 focus:ring-brand/20 focus:border-brand outline-none"
                  />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-gray-700 mb-1">Nama / Judul Ibadah</label>
                  <input
                    type="text"
                    required
                    value={editingItem.title || ''}
                    onChange={(e) => setEditingItem({ ...editingItem, title: e.target.value })}
                    placeholder="Contoh: Ibadah Raya 1 atau Youth Fellowship"
                    className="w-full p-2.5 bg-white border border-gray-200 rounded-xl text-xs font-medium focus:ring-2 focus:ring-brand/20 focus:border-brand outline-none"
                  />
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-semibold text-gray-700 mb-1">Waktu Pelaksanaan</label>
                  <input
                    type="text"
                    required
                    value={editingItem.time || ''}
                    onChange={(e) => setEditingItem({ ...editingItem, time: e.target.value })}
                    placeholder="Contoh: 07.00 WIB atau 19.00 WIB"
                    className="w-full p-2.5 bg-white border border-gray-200 rounded-xl text-xs font-medium focus:ring-2 focus:ring-brand/20 focus:border-brand outline-none"
                  />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-gray-700 mb-1">Hari / Rutinitas Pelaksanaan</label>
                  <input
                    type="text"
                    value={editingItem.dateDay || ''}
                    onChange={(e) => setEditingItem({ ...editingItem, dateDay: e.target.value })}
                    placeholder="Contoh: Setiap Hari Minggu atau Setiap Selasa"
                    className="w-full p-2.5 bg-white border border-gray-200 rounded-xl text-xs font-medium focus:ring-2 focus:ring-brand/20 focus:border-brand outline-none"
                  />
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-semibold text-gray-700 mb-1">Pembicara / Pengkhotbah</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-xs font-semibold text-gray-700 mb-1">Petugas WL (Worship Leader)</label>
                  <input
                    type="text"
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-semibold text-gray-700 mb-1">Lokasi / Ruangan Ibadah</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-xs font-semibold text-gray-700 mb-1">Kategori Ibadah</label>
                  <select
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="Contoh: Disertai Perjamuan Kudus pada minggu pertama"
                  className="w-full p-2.5 bg-white border border-gray-200 rounded-xl text-xs font-medium focus:ring-2 focus:ring-brand/20 focus:border-brand outline-none"
                />
              </div>
              ...
              <div className="pt-1">
                <label className="flex items-center gap-2 cursor-pointer select-none">
                  ...
                  />
                  <span className="text-xs font-semibold text-gray-800">Tersedia Live Streaming Online (YouTube / Zoom)</span>
                </label>
              </div>

              {/* LIVE PREVIEW OF SCHEDULE CARD */}
              <div className="mt-4 p-4 bg-amber-50/40 border border-amber-200/70 rounded-2xl space-y-2">
                <div className="flex items-center justify-between text-[10px] font-bold text-amber-800 uppercase tracking-wider">
                  <span>📱 Pratinjau Tampilan di Dashboard Jemaat:</span>
                  <span className="bg-amber-100 text-amber-900 px-2 py-0.5 rounded-md">Live Preview</span>
                </div>
                <div className="p-4 bg-white border border-gray-200 rounded-xl shadow-sm space-y-2.5">
                  <div className="flex items-center justify-between gap-2">
                    <span className="px-2.5 py-0.5 bg-brand text-white text-[9px] font-black rounded-full uppercase tracking-wider">
                      {editingItem.sessionName || 'NAMA SESI'}
                    </span>
                    <span className="text-[10px] font-mono font-bold text-gray-400">
                      {editingItem.dateDay || 'Setiap Minggu'}
                    </span>
                  </div>
                  <h4 className="font-display font-bold text-gray-800 text-sm">
                    {editingItem.title || 'Judul Ibadah'}
                  </h4>
                  <p className="text-xl font-bold font-mono text-brand">
                    {editingItem.time || '00.00 WIB'}
                  </p>
                  <div className="text-xs text-gray-600 space-y-1 pt-2 border-t border-gray-100">
                    {editingItem.speaker && <p><strong>Pembicara:</strong> {editingItem.speaker}</p>}
                    {editingItem.worshipLeader && <p><strong>Petugas WL:</strong> {editingItem.worshipLeader}</p>}
                    {editingItem.location && <p><strong>Lokasi:</strong> {editingItem.location}</p>}
                    {editingItem.category && <p><strong>Kategori:</strong> {editingItem.category}</p>}
                    {editingItem.isOnline && (
                      <p className="text-emerald-600 font-bold flex items-center gap-1 text-[11px]">
                        🔴 Live Streaming Tersedia
                      </p>
                    )}
                    {editingItem.notes && <p className="text-[11px] text-gray-400 italic">{editingItem.notes}</p>}
                  </div>
                </div>
              </div>
        ...
        <div className="space-y-4">
          ...
          {activeTab === 'admin_schedules' && (
            <div className="space-y-4 pt-2">
              <div className="p-4 bg-amber-50/40 border border-amber-200/60 rounded-2xl flex items-center justify-between flex-wrap gap-2">
                <div>
                  <h4 className="font-bold text-xs text-amber-900 uppercase tracking-wide">Daftar Jadwal Ibadah Terdaftar ({schedules.length})</h4>
                  <p className="text-[11px] text-amber-700">Setiap perubahan atau penghapusan jadwal di sini langsung terhubung secara otomatis dengan Dashboard Jemaat.</p>
                </div>
                <div className="flex items-center gap-2">
                  {schedules.length > 0 && (
                    <button
                    ...
                    >
                      <Trash2 className="w-3.5 h-3.5 text-red-600" /> Bersihkan Semua Jadwal
                    </button>
                  )}
                  {!editingItem && (
                    <button
                      ...
                      className="px-3.5 py-2 bg-brand text-white text-xs font-bold rounded-xl flex items-center gap-1.5 hover:bg-brand-dark transition-colors cursor-pointer shadow-sm"
                    >
              ...
              {schedules.length === 0 ? (
                <div className="p-8 text-center bg-gray-50/60 rounded-2xl border border-gray-200/80">
                  <p className="text-xs text-gray-500 font-medium">Belum ada jadwal ibadah yang ditambahkan. Klik "Tambah Jadwal Baru" di atas untuk menambahkan jadwal pertama.</p>
                </div>
              ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {schedules.map((sch) => (
                    <div key={sch.id} className="p-5 bg-white border border-gray-200/90 rounded-2xl space-y-3.5 shadow-sm hover:border-brand hover:shadow-md transition-all flex flex-col justify-between">
                      <div className="space-y-2.5">
                        <div className="flex items-center justify-between gap-2">
                          <span className="px-2.5 py-0.5 bg-brand text-white text-[9px] font-black rounded-full uppercase tracking-wider">
                            {sch.sessionName || 'IBADAH'}
                          </span>
                          <span className="text-[10px] font-mono font-bold text-gray-400">
                        ...
                        <p className="text-2xl font-bold font-mono text-brand">{sch.time}</p>
                        ...
                        <div className="text-xs text-gray-600 space-y-1.5 pt-3 border-t border-gray-100">
                          ...
                          {sch.category && <p><strong>Kategori:</strong> <span className="bg-gray-100 px-1.5 py-0.5 rounded text-[10px] font-semibold text-gray-700">{sch.category}</span></p>}
                          {sch.isOnline && (
                            <p className="text-emerald-600 font-bold flex items-center gap-1 text-[11px]">
                              🔴 Live Streaming Tersedia
                            </p>
                          )}
                          {sch.notes && <p className="text-[11px] text-gray-400 italic pt-0.5">{sch.notes}</p>}
                        </div>
                      </div>
                      ...
                      <div className="flex items-center justify-end gap-2 pt-3 border-t border-gray-100">
                        <button
                          ...
                          className="px-3.5 py-2 bg-gray-100 hover:bg-teal-50 hover:text-teal-700 text-gray-700 font-bold text-xs rounded-xl flex items-center gap-1.5 transition-colors cursor-pointer"
                        >
                        ...
                        <button
                          ...
                          className="px-3.5 py-2 bg-red-50 hover:bg-red-100 text-red-700 font-bold text-xs rounded-xl flex items-center gap-1.5 transition-colors cursor-pointer border border-red-200"
                        >
                          <Trash2 className="w-3.5 h-3.5 text-red-600" /> Hapus
                        </button>
                      </div>
                    </div>
                  ))}
      ...
      {/* Moderasi Komentar */}
          {activeTab === 'admin_comments' && (
            <div className="space-y-5">
              <div className="p-4 bg-gradient-to-r from-amber-500/10 via-amber-50 to-white text-amber-950 border border-amber-200/80 rounded-2xl flex items-start gap-3 shadow-sm">
                <AlertCircle className="w-5 h-5 text-amber-600 flex-shrink-0 mt-0.5" />
                <div className="space-y-1">
                  <h4 className="font-display font-bold text-xs uppercase tracking-wider text-amber-900">
                    Sistem Moderasi Komentar Jemaat (Aktif)
                  </h4>
                  <p className="text-xs leading-relaxed text-amber-800">
                    <strong>Fungsi Moderasi Komentar:</strong> Meninjau dan menyaring setiap komentar yang dikirimkan jemaat pada Berita & Renungan Harian sebelum ditampilkan secara umum di portal. Komentar jemaat baru akan berstatus <span className="bg-amber-200/60 px-1.5 py-0.5 rounded font-bold text-amber-900">Pending</span>. Anda dapat menyetujui (<strong>Approve</strong>) agar tayang di Dashboard Jemaat, menandai <strong>SPAM</strong>, atau <strong>Menghapus</strong> komentar.
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-2 overflow-x-auto pb-1">
                {[
                  ...
                  { id: 'pending', label: 'Menunggu Moderasi (Pending)' },
                  { id: 'approved', label: 'Disetujui (Approved)' },
                  { id: 'spam', label: 'Spam / Ditolak' },
                ].map((f) => (
                  <button
                    key={f.id}
                  ...
                  >
                    {f.label}
                    {f.id === 'pending' && (
                      <span className="ml-1.5 px-1.5 py-0.2 bg-amber-500 text-slate-950 rounded-full text-[9px] font-black">
                        {comms.filter(c => c.status === 'pending').length}
                      </span>
                    )}
                  </button>
                ))}
              ...
              <div className="space-y-3">
                {comms.filter(c => categoryFilter === 'all' || categoryFilter === 'Semua' || c.status === categoryFilter).length === 0 ? (
                  <div className="text-center py-12 bg-gray-50 border border-gray-100 rounded-2xl">
                    <MessageSquare className="w-8 h-8 text-gray-300 mx-auto mb-2" />
                    <p className="text-gray-500 text-xs font-medium">Tidak ada komentar pada filter status ini.</p>
                  </div>
                ) : (
                  comms
                    .filter(c => categoryFilter === 'all' || categoryFilter === 'Semua' || c.status === categoryFilter)
                    .map((item) => (
                      <div key={item.id} className="p-4 bg-white border border-gray-200/80 rounded-2xl space-y-3 shadow-sm hover:border-brand/50 transition-all">
                        <div className="flex items-start justify-between gap-3 flex-wrap">
                          <div className="space-y-0.5">
                            <strong className="text-xs text-gray-900 font-bold">{item.userName}</strong>
                            <span className="text-[10px] text-gray-400 block font-mono">Email: {item.userEmail || '-'}</span>
                            <span className="text-[10px] font-medium text-brand block mt-1 bg-teal-50 px-2 py-0.5 rounded w-max">
                              Pada: {item.targetTitle} ({item.targetType === 'news' ? 'Berita' : 'Renungan'})
                            </span>
                          </div>
                          <div className="flex items-center gap-2">
                            ...
                            <span className={`px-2.5 py-0.5 text-[9px] font-black rounded-full uppercase tracking-wider ${
                              item.status === 'approved' ? 'bg-emerald-100 text-emerald-800 border border-emerald-200' : item.status === 'spam' ? 'bg-rose-100 text-rose-800 border border-rose-200' : 'bg-amber-100 text-amber-900 border border-amber-200'
                            }`}>
                              {item.status === 'approved' ? '✓ Approved' : item.status === 'spam' ? '✗ Spam' : '⏳ Pending'}
                            </span>
                          </div>
                        </div>

                        <div className="p-3 bg-gray-50 rounded-xl border border-gray-100">
                          <p className="text-xs text-gray-800 leading-relaxed italic">"{item.content}"</p>
                        </div>

                        <div className="pt-2 border-t border-gray-100 flex items-center gap-2 justify-end">
                          {item.status !== 'approved' && (
                            <button
                              onClick={() => handleModerateComment(item.id, 'approved')}
                              className="px-3.5 py-1.5 bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold rounded-xl transition-all shadow-sm flex items-center gap-1 cursor-pointer"
                            >
                              <Check className="w-3.5 h-3.5 text-white" /> Setujui (Approve)
                            </button>
                          )}
                          {item.status !== 'spam' && (
                            <button
                              onClick={() => handleModerateComment(item.id, 'spam')}
                              className="px-3.5 py-1.5 bg-amber-50 hover:bg-amber-100 text-amber-800 border border-amber-200 text-xs font-bold rounded-xl transition-all flex items-center gap-1 cursor-pointer"
                            >
                              <AlertCircle className="w-3.5 h-3.5 text-amber-600" /> Tandai SPAM
                            </button>
                          )}
                          <button
                            onClick={() => {
                              if (confirm('Hapus komentar ini secara permanen dari database?')) {
                                MockDatabase.deleteComment(item.id, currentUser);
                                loadAllData();
                                showToast('Komentar berhasil dihapus.', 'info');
                              }
                            }}
                            className="px-3.5 py-1.5 bg-rose-50 hover:bg-rose-100 border border-rose-200 text-rose-700 text-xs font-bold rounded-xl transition-all cursor-pointer flex items-center gap-1"
                          >
          ...
          {/* Kelola Donasi & Kas Gereja */}
          {activeTab === 'admin_donations' && (
            <div className="space-y-6">
              {/* Header Overview Card */}
              <div className="p-5 bg-gradient-to-r from-emerald-900 via-teal-900 to-slate-900 text-white rounded-3xl shadow-lg border border-emerald-800/40 relative overflow-hidden">
                <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
                  <div className="space-y-1">
                    <div className="flex items-center gap-2 text-emerald-400">
                      <Coins className="w-5 h-5" />
                      <span className="text-[10px] font-black uppercase tracking-widest bg-emerald-950/80 px-2.5 py-0.5 rounded-full border border-emerald-700/50">
                        Sistem Laporan Kas & Donasi Gereja
                      </span>
                    </div>
                    <h3 className="font-display font-black text-xl text-white tracking-tight">
                      Manajemen Kas, Persepuluhan & Verification Donasi
                    </h3>
                    <p className="text-xs text-slate-300 max-w-xl leading-relaxed">
                      Laporan kas terintegrasi real-time dari konfirmasi jemaat. Verifikasi transfer bank & QRIS, catat kas fisik/tunai, serta ekspor laporan keuangan untuk audit internal bendahara.
                    </p>
                  </div>

                  <div className="flex flex-wrap items-center gap-2">
                    <button
                      onClick={() => setIsAddingManualDonation(true)}
                      className="px-3.5 py-2.5 bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-black text-xs rounded-2xl shadow-md transition-all flex items-center gap-2 cursor-pointer"
                    >
                      <Plus className="w-4 h-4" /> Catat Kas Tunai / Offline
                    </button>
                    <button
                      onClick={() => setIsEditingBankSettings(true)}
                      className="px-3.5 py-2.5 bg-slate-800 hover:bg-slate-700 text-slate-100 font-bold text-xs rounded-2xl border border-slate-700 transition-all flex items-center gap-2 cursor-pointer"
                    >
                      <Settings className="w-4 h-4 text-emerald-400" /> Rekening & QRIS
                    </button>
                    <button
                      onClick={handleExportDonationsCSV}
                      className="px-3.5 py-2.5 bg-white/10 hover:bg-white/20 text-white font-bold text-xs rounded-2xl border border-white/20 transition-all flex items-center gap-2 cursor-pointer"
                    >
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mt-6 pt-5 border-t border-slate-800/80">
                  <div className="p-3 bg-slate-950/60 rounded-2xl border border-slate-800/80">
                    <span className="text-[9px] font-extrabold uppercase text-slate-400">Total Kas Terverifikasi</span>
                    <p className="text-lg font-black text-emerald-400 font-mono mt-0.5">
                      Rp {donations.filter(d => d.status === 'verified').reduce((s, d) => s + (d.amount || 0), 0).toLocaleString('id-ID')}
                    </p>
                  </div>
                  <div className="p-3 bg-slate-950/60 rounded-2xl border border-amber-900/40">
                    <span className="text-[9px] font-extrabold uppercase text-amber-400">Menunggu Verifikasi</span>
                    <p className="text-lg font-black text-amber-400 font-mono mt-0.5">
                      {donations.filter(d => d.status === 'pending').length} Trx (Rp {donations.filter(d => d.status === 'pending').reduce((s, d) => s + (d.amount || 0), 0).toLocaleString('id-ID')})
                    </p>
                  </div>
                  <div className="p-3 bg-slate-950/60 rounded-2xl border border-slate-800/80">
                    <span className="text-[9px] font-extrabold uppercase text-slate-400">Persepuluhan (Verified)</span>
                    <p className="text-lg font-black text-indigo-400 font-mono mt-0.5">
                      Rp {donations.filter(d => d.status === 'verified' && d.type === 'Persepuluhan').reduce((s, d) => s + (d.amount || 0), 0).toLocaleString('id-ID')}
                    </p>
                  </div>
                  <div className="p-3 bg-slate-950/60 rounded-2xl border border-slate-800/80">
                    <span className="text-[9px] font-extrabold uppercase text-slate-400">Pembangunan & Diakonia</span>
                    <p className="text-lg font-black text-blue-400 font-mono mt-0.5">
                      Rp {donations.filter(d => d.status === 'verified' && (d.type === 'Donasi Pembangunan' || d.type === 'Diakonia / Sosial')).reduce((s, d) => s + (d.amount || 0), 0).toLocaleString('id-ID')}
                    </p>
                  </div>
                </div>
              </div>

              {/* Bank & QRIS Settings Modal */}
              {isEditingBankSettings && (
                <div className="p-5 bg-amber-50/50 dark:bg-slate-900 border border-amber-200 dark:border-slate-800 rounded-3xl space-y-4 shadow-sm">
                  <div className="flex items-center justify-between border-b border-amber-200/60 dark:border-slate-800 pb-3">
                    <div className="flex items-center gap-2 text-amber-900 dark:text-amber-400 font-bold text-sm">
                      <QrCode className="w-5 h-5 text-amber-600" />
                      <span>Atur Rekening Bank Resmi & QRIS Gereja</span>
                    </div>
                    <button onClick={() => setIsEditingBankSettings(false)} className="p-1 text-gray-400 hover:text-gray-600 rounded-lg">
                      <X className="w-4 h-4" />
                    </button>
                  </div>

                  <form onSubmit={handleSaveBankSettings} className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-[10px] font-bold text-gray-600 dark:text-gray-400 uppercase mb-1">Nama Bank</label>
                      <input
                        type="text"
                        required
                        value={bankSettingsForm.bankName}
                        onChange={(e) => setBankSettingsForm({ ...bankSettingsForm, bankName: e.target.value })}
                        placeholder="Contoh: BCA (Bank Central Asia)"
                        className="w-full p-2.5 border border-gray-200 dark:border-slate-800 rounded-xl text-xs bg-white dark:bg-slate-950"
                      />
                    </div>
                    <div>
                      <label className="block text-[10px] font-bold text-gray-600 dark:text-gray-400 uppercase mb-1">Nomor Rekening</label>
                      <input
                        type="text"
                        required
                        value={bankSettingsForm.bankAccountNo}
                        onChange={(e) => setBankSettingsForm({ ...bankSettingsForm, bankAccountNo: e.target.value })}
                        placeholder="Contoh: 8290123456"
                        className="w-full p-2.5 border border-gray-200 dark:border-slate-800 rounded-xl text-xs font-mono font-bold bg-white dark:bg-slate-950"
                      />
                    </div>
                    <div>
                      <label className="block text-[10px] font-bold text-gray-600 dark:text-gray-400 uppercase mb-1">Atas Nama Rekening</label>
                      <input
                        type="text"
                        required
                        value={bankSettingsForm.bankAccountName}
                        onChange={(e) => setBankSettingsForm({ ...bankSettingsForm, bankAccountName: e.target.value })}
                        placeholder="Contoh: SYSTEM MANAGEMENT CHURCH"
                        className="w-full p-2.5 border border-gray-200 dark:border-slate-800 rounded-xl text-xs bg-white dark:bg-slate-950"
                      />
                    </div>
                    <div>
                      <label className="block text-[10px] font-bold text-gray-600 dark:text-gray-400 uppercase mb-1">URL / Link Barcode QRIS</label>
                      <input
                        type="text"
                        value={bankSettingsForm.qrisUrl}
                        onChange={(e) => setBankSettingsForm({ ...bankSettingsForm, qrisUrl: e.target.value })}
                        placeholder="Paste URL gambar QRIS..."
                        className="w-full p-2.5 border border-gray-200 dark:border-slate-800 rounded-xl text-xs bg-white dark:bg-slate-950"
                      />
                    </div>
                    <div className="md:col-span-2 flex items-center justify-end gap-2 pt-2">
                      <button
                        ...
                        className="px-4 py-2 border border-gray-200 text-gray-600 text-xs font-bold rounded-xl hover:bg-gray-100 cursor-pointer"
                      >
                        Batal
                      </button>
                      <button
                        type="submit"
                        className="px-5 py-2 bg-amber-600 hover:bg-amber-700 text-white text-xs font-bold rounded-xl shadow-sm cursor-pointer"
                      >
              ...
              {/* Manual Donation Input Modal */}
              {isAddingManualDonation && (
                <div className="p-5 bg-emerald-50/50 dark:bg-slate-900 border border-emerald-200 dark:border-slate-800 rounded-3xl space-y-4 shadow-sm">
                  <div className="flex items-center justify-between border-b border-emerald-200/60 dark:border-slate-800 pb-3">
                    <div className="flex items-center gap-2 text-emerald-900 dark:text-emerald-400 font-bold text-sm">
                      <Plus className="w-5 h-5 text-emerald-600" />
                      <span>Pencatatan Kas Tunai / Ampop Persembahan Offline</span>
                    </div>
                    <button onClick={() => setIsAddingManualDonation(false)} className="p-1 text-gray-400 hover:text-gray-600 rounded-lg">
                      <X className="w-4 h-4" />
                    </button>
                  </div>

                  <form onSubmit={handleSaveManualDonation} className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-[10px] font-bold text-gray-600 dark:text-gray-400 uppercase mb-1">Nama Donatur / Keterangan</label>
                      <input
                        type="text"
                        required
                        value={manualDonationForm.donorName}
                        onChange={(e) => setManualDonationForm({ ...manualDonationForm, donorName: e.target.value })}
                        placeholder="Contoh: Kolekte Ibadah Raya Minggu 1"
                        className="w-full p-2.5 border border-gray-200 dark:border-slate-800 rounded-xl text-xs bg-white dark:bg-slate-950"
                      />
                    </div>
                    <div>
                      <label className="block text-[10px] font-bold text-gray-600 dark:text-gray-400 uppercase mb-1">Nominal Donasi (Rp)</label>
                      <input
                        type="number"
                        required
                    ...
                    <div>
                      <label className="block text-[10px] font-bold text-gray-600 dark:text-gray-400 uppercase mb-1">Jenis Persembahan</label>
                      <select
                        value={manualDonationForm.type}
                        onChange={(e) => setManualDonationForm({ ...manualDonationForm, type: e.target.value })}
                        className="w-full p-2.5 border border-gray-200 dark:border-slate-800 rounded-xl text-xs bg-white dark:bg-slate-950"
                      >
                        <option value="Persepuluhan">Persepuluhan</option>
                        <option value="Persembahan Kas Kasih">Persembahan Kas Kasih</option>
                        <option value="Donasi Pembangunan">Donasi Pembangunan</option>
                        <option value="Diakonia / Sosial">Diakonia / Sosial</option>
                        <option value="Kolekte Minggu">Kolekte Minggu</option>
                      </select>
                    </div>
                    <div>
                      <label className="block text-[10px] font-bold text-gray-600 dark:text-gray-400 uppercase mb-1">Metode Pembayaran</label>
                      <select
                        value={manualDonationForm.paymentMethod}
                        onChange={(e) => setManualDonationForm({ ...manualDonationForm, paymentMethod: e.target.value })}
                        className="w-full p-2.5 border border-gray-200 dark:border-slate-800 rounded-xl text-xs bg-white dark:bg-slate-950"
                      >
                        <option value="Tunai / Offline">Tunai / Offline (Ampop)</option>
                        <option value="Transfer Bank">Transfer Bank Manual</option>
                        <option value="QRIS">QRIS Direct Scan</option>
                      </select>
                    </div>
                    <div className="md:col-span-2">
                      <label className="block text-[10px] font-bold text-gray-600 dark:text-gray-400 uppercase mb-1">Catatan Tambahan</label>
                      <input
                        type="text"
                        value={manualDonationForm.note}
                        onChange={(e) => setManualDonationForm({ ...manualDonationForm, note: e.target.value })}
                        placeholder="Contoh: Hasil ampop ibadah pukul 09.00 WIB..."
                        className="w-full p-2.5 border border-gray-200 dark:border-slate-800 rounded-xl text-xs bg-white dark:bg-slate-950"
                      />
                    </div>
                    <div className="md:col-span-2 flex items-center justify-end gap-2 pt-2">
              ...
              {/* Filters Bar */}
              <div className="flex flex-col md:flex-row items-stretch md:items-center justify-between gap-4 bg-gray-50/50 dark:bg-slate-900/60 p-4 rounded-2xl border border-gray-100 dark:border-slate-800">
                {/* Status Tabs */}
                <div className="flex items-center gap-1.5 overflow-x-auto pb-1 md:pb-0">
                  {[
                    ...
                    <button
                      key={tab.id}
                      onClick={() => setDonationFilterStatus(tab.id as any)}
                      className={`px-3 py-1.5 rounded-xl text-xs font-bold transition-all whitespace-nowrap cursor-pointer ${
                        donationFilterStatus === tab.id
                ...
                <div className="flex items-center gap-2">
                  <select
                    ...
                    className="p-2 border border-gray-200 dark:border-slate-800 rounded-xl text-xs bg-white dark:bg-slate-900 font-semibold"
                  >
                    <option value="Semua">Semua Kategori Persembahan</option>
                    <option value="Persepuluhan">Persepuluhan</option>
                    <option value="Persembahan Kas Kasih">Persembahan Kas Kasih</option>
                    <option value="Donasi Pembangunan">Donasi Pembangunan</option>
                    <option value="Diakonia / Sosial">Diakonia / Sosial</option>
                  ...
                  <div className="relative flex-1 md:w-48">
                    ...
                    <input
                      ...
                      className="w-full pl-8 pr-3 py-1.5 border border-gray-200 dark:border-slate-800 rounded-xl text-xs bg-white dark:bg-slate-900"
                    />
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white dark:bg-slate-900 border border-gray-100 dark:border-slate-800 rounded-3xl overflow-hidden shadow-sm">
                <div className="overflow-x-auto">
                  <table className="w-full text-left text-xs">
                    <thead className="bg-gray-50 dark:bg-slate-950/50 text-gray-500 uppercase text-[10px] font-black border-b border-gray-100 dark:border-slate-800 tracking-wider">
                      <tr>
                        <th className="p-3.5">Tanggal / ID</th>
                        <th className="p-3.5">Nama Donatur</th>
                        <th className="p-3.5">Jenis / Metode</th>
                        <th className="p-3.5">Nominal (Rp)</th>
                        <th className="p-3.5">Catatan / Doa</th>
                    ...
                    <tbody className="divide-y divide-gray-100 dark:divide-slate-800">
                      {donations
                        .filter((d) => {
                          ...
                          const matchType = donationFilterType === 'Semua' || d.type === donationFilterType;
                        ...
                        .map((item) => (
                          <tr key={item.id} className="hover:bg-gray-50/60 dark:hover:bg-slate-800/40 transition-colors">
                            <td className="p-3.5 font-mono text-[11px] text-gray-500 whitespace-nowrap">
                              <div>{new Date(item.date).toLocaleDateString('id-ID', { day: '2-digit', month: 'short', year: 'numeric' })}</div>
                              <span className="text-[9px] text-gray-400">{new Date(item.date).toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' })}</span>
                            </td>
                            <td className="p-3.5">
                              <div className="font-bold text-gray-800 dark:text-gray-100">{item.donorName}</div>
                              {item.phone && <div className="text-[10px] text-gray-400">{item.phone}</div>}
                            </td>
                            <td className="p-3.5">
                              <span className="inline-block font-extrabold text-indigo-600 dark:text-indigo-400">{item.type}</span>
                              <div className="text-[10px] text-gray-400 font-medium">{item.paymentMethod || 'Transfer Bank'}</div>
                            </td>
                            <td className="p-3.5 font-mono font-black text-sm text-emerald-600 dark:text-emerald-400 whitespace-nowrap">
                              Rp {item.amount.toLocaleString('id-ID')}
                            </td>
                            <td className="p-3.5 max-w-xs">
                              {item.note ? (
                                <p className="text-[11px] text-gray-600 dark:text-gray-300 italic bg-gray-50 dark:bg-slate-950 p-2 rounded-xl border border-gray-100 dark:border-slate-800">
                                  "{item.note}"
                                </p>
                              ) : (
                            ...
                            <td className="p-3.5 whitespace-nowrap">
                              {item.status === 'verified' && (
                                <span className="inline-flex items-center gap-1 bg-emerald-50 text-emerald-700 dark:bg-emerald-950/80 dark:text-emerald-400 px-2.5 py-1 rounded-full text-[10px] font-black border border-emerald-200/80 dark:border-emerald-800">
                                  <CheckCircle2 className="w-3 h-3 text-emerald-600" /> Terverifikasi
                                </span>
                              )}
                              {item.status === 'pending' && (
                                <span className="inline-flex items-center gap-1 bg-amber-50 text-amber-700 dark:bg-amber-950/80 dark:text-amber-400 px-2.5 py-1 rounded-full text-[10px] font-black border border-amber-200/80 dark:border-amber-800 animate-pulse">
                                  ⏳ Menunggu
                                </span>
                              )}
                              {item.status === 'rejected' && (
                                <span className="inline-flex items-center gap-1 bg-rose-50 text-rose-700 dark:bg-rose-950/80 dark:text-rose-400 px-2.5 py-1 rounded-full text-[10px] font-black border border-rose-200/80 dark:border-rose-800">
                                  ✗ Ditolak
                                </span>
                              )}
                              {item.verifiedBy && (
                                <div className="text-[9px] text-gray-400 mt-1">oleh {item.verifiedBy}</div>
                              )}
                            </td>
                              ...
                              <div className="flex items-center justify-end gap-1.5">
                                {item.status !== 'verified' && (
                                  <button
                                    ...
                                    className="px-2.5 py-1 bg-emerald-500 hover:bg-emerald-600 text-white text-[10px] font-bold rounded-lg shadow-sm cursor-pointer"
                                    title="Setujui & Verifikasi Kas"
                                  >
                                    ✓ Verifikasi
                                  </button>
                                )}
                                ...
                                <button
                                  onClick={() => setDeleteModal({ isOpen: true, type: 'donation', id: item.id, title: `Donasi Rp ${item.amount.toLocaleString('id-ID')} (${item.donorName})` })}
                                  className="p-1 text-gray-400 hover:text-rose-600 hover:bg-rose-50 rounded-lg transition-colors cursor-pointer"
                                  title="Hapus Record Donasi"
                                >
                                  <Trash2 className="w-3.5 h-3.5" />
                                </button>
                              </div>
                  ...
                  {donations.length === 0 && (
                    <div className="py-12 text-center text-gray-400 space-y-2">
                      <Coins className="w-8 h-8 mx-auto text-gray-300" />
                      <p className="text-xs font-semibold">Belum ada catatan donasi atau kas terdaftar.</p>
                    </div>
                  )}
                </div>
              </div>
