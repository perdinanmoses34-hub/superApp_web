...
export default function DashboardAdmin({ setTab, currentUser, settings }: DashboardAdminProps) {
  ...
  const loadData = () => {
    ...
    const dons = MockDatabase.getDonations();

    const verifiedSum = dons.filter((d) => d.status === 'verified').reduce((sum, d) => sum + (d.amount || 0), 0);
    const pendingCount = dons.filter((d) => d.status === 'pending').length;
    ...
    setStats({
      ...
      totalVerifiedDonations: verifiedSum,
  ...
  return (
    <div className="bg-[#070B14] p-6 -mx-6 md:-mx-8 -my-6 md:-my-8 min-h-screen text-slate-100 space-y-8 animate-fade-in">
      ...
      {/* Alert Banner for Pending Donations */}
      {stats.pendingDonationsCount > 0 && (
        <div className="bg-gradient-to-r from-amber-500/20 via-orange-500/20 to-amber-600/20 border border-amber-500/40 p-4 sm:p-5 rounded-3xl flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 shadow-lg animate-pulse">
          <div className="flex items-center gap-3">
            <div className="p-3 bg-amber-500/30 text-amber-300 rounded-2xl border border-amber-400/40 flex-shrink-0">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-bold text-amber-300">
                  {stats.pendingDonationsCount} Laporan Donasi / Kas Masuk
                </span>
              </div>
              <p className="text-xs text-slate-200 mt-1 font-medium">
                Ada konfirmasi persembahan/kas jemaat yang baru dikirimkan dan menunggu verifikasi bendahara gereja.
              </p>
            </div>
          </div>
          <button
            onClick={() => setTab('admin_donations')}
            className="px-5 py-2.5 bg-amber-400 hover:bg-amber-300 text-slate-950 font-black text-xs rounded-2xl shadow-md transition-all flex items-center gap-2 cursor-pointer self-stretch sm:self-auto justify-center"
          >
            <Coins className="w-4 h-4" /> Verifikasi Sekarang →
          </button>
        </div>
      )}
      ...
      <div className="bg-[#0F172A] text-white p-3.5 sm:p-4 rounded-3xl shadow-lg border border-slate-800">
        ...
        <div className="flex items-center gap-2 overflow-x-auto pb-1 no-scrollbar scroll-smooth">
          {[
            ...
            { id: 'admin_donations', label: `💰 Kas & Donasi (${stats.pendingDonationsCount > 0 ? stats.pendingDonationsCount + ' Menunggu' : 'Ok'})` },
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
        ...
        <div className="bg-[#0F172A]/90 border border-slate-800/80 p-5 rounded-3xl relative overflow-hidden flex flex-col justify-between min-h-[140px] group hover:scale-[1.02] transition-all shadow-lg">
          ...
          <p className="text-3xl font-black text-white mt-2">{stats.members}</p>
        ...
        <button
          onClick={() => setTab('admin_donations')}
          className="bg-[#0F172A]/90 border border-slate-800/80 hover:border-emerald-500/50 p-5 rounded-3xl relative overflow-hidden flex flex-col justify-between min-h-[140px] group hover:scale-[1.02] transition-all shadow-lg text-left cursor-pointer"
        >
          <div className="flex items-center justify-between">
            <p className="text-slate-400 font-extrabold text-[10px] uppercase tracking-widest">Kas & Donasi</p>
            <div className="p-2 bg-emerald-500/10 text-emerald-400 rounded-xl border border-emerald-500/20">
              <Coins className="w-4 h-4" />
            </div>
          </div>
          <p className="text-xl md:text-2xl font-black text-emerald-400 mt-2 font-mono truncate">
            Rp {stats.totalVerifiedDonations.toLocaleString('id-ID')}
          </p>
          <span className={`text-[10px] px-2.5 py-1 rounded-full font-black w-fit uppercase border mt-3 flex items-center gap-1 ${
            stats.pendingDonationsCount > 0
              ? 'bg-amber-500/20 text-amber-300 border-amber-500/30 animate-pulse'
              : 'bg-emerald-500/15 text-emerald-400 border-emerald-500/20'
          }`}>
            {stats.pendingDonationsCount > 0 ? `⏳ ${stats.pendingDonationsCount} Menunggu Verifikasi` : '✓ Audit Kas Lengkap'}
          </span>
        </button>
        ...
        <div className="bg-[#0F172A]/90 border border-slate-800/80 p-5 rounded-3xl relative overflow-hidden flex flex-col justify-between min-h-[140px] group hover:scale-[1.02] transition-all shadow-lg">
          ...
          <p className="text-3xl font-black text-white mt-2">{stats.events}</p>
        ...
        <div className="bg-[#0F172A]/90 border border-slate-800/80 p-5 rounded-3xl relative overflow-hidden flex flex-col justify-between min-h-[140px] group hover:scale-[1.02] transition-all shadow-lg">
          ...
          <p className="text-3xl font-black text-white mt-2">{stats.announcements + stats.news}</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-[#0F172A]/90 border border-slate-800/80 p-6 rounded-3xl shadow-xl space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <button
              onClick={() => setTab('admin_donations')}
              className="p-3 bg-slate-900/50 hover:bg-slate-800/50 border border-slate-800/60 hover:border-emerald-500 rounded-xl text-left transition-all group cursor-pointer col-span-2 sm:col-span-1"
            >
              <Coins className="w-5 h-5 text-emerald-400" />
              <p className="text-xs font-bold text-slate-200 mt-2 group-hover:text-emerald-400">Kas & Persepuluhan</p>
              <span className="text-[9px] text-slate-500">
                {stats.pendingDonationsCount > 0 ? `⚠️ ${stats.pendingDonationsCount} Menunggu Verifikasi` : 'Verifikasi Donasi'}
              </span>
            </button>
