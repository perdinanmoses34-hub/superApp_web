...
const DEFAULT_SETTINGS: ChurchSettings = {
  ...
  jemaatWelcomeTitle: 'SELAMAT DATANG DI SYSTEM MANAJEMEN CHURCH',
...
const DEFAULT_DONATIONS: Donation[] = [
  {
    ...
    amount: 3500000,
    type: 'Persepuluhan',
    note: 'Persepuluhan bulan Juli 2026. Terima kasih Tuhan Yesus.',
    paymentMethod: 'Transfer Bank',
    date: '2026-07-20T09:30:00.000Z',
    status: 'verified',
    verifiedBy: 'Pnt. Budi Santoso',
    verifiedDate: '2026-07-20T10:15:00.000Z',
  },
  {
    id: 'don_2',
    ...
    phone: '081234567802',
    amount: 2500000,
    type: 'Donasi Pembangunan',
    note: 'Dukungan dana renovasi ruang sound system & instrumen gereja.',
    paymentMethod: 'QRIS',
    date: '2026-07-21T14:20:00.000Z',
    status: 'verified',
    verifiedBy: 'Pnt. Budi Santoso',
    verifiedDate: '2026-07-21T15:00:00.000Z',
  },
  {
    id: 'don_3',
    donorName: 'Kel. Bpk. Daniel Setiawan',
    userId: 'usr_guest_1',
    userEmail: 'daniel@gmail.com',
    phone: '081199887766',
    amount: 750000,
    type: 'Diakonia / Sosial',
    note: 'Persembahan kasih untuk aksi donor darah & pengobatan gratis jemaat.',
    paymentMethod: 'Transfer Bank',
    date: '2026-07-22T08:10:00.000Z',
    status: 'verified',
    verifiedBy: 'Bendahara Gereja',
    verifiedDate: '2026-07-22T08:30:00.000Z',
  },
  {
    id: 'don_4',
    donorName: 'Kas Ampop Ibadah Minggu',
    userId: 'usr_admin',
    userEmail: 'kasir@church.com',
    phone: '081234567890',
    amount: 1000000,
    type: 'Kolekte Minggu',
    note: 'Pencatatan ampop fisik ibadah raya 1 & 2',
...
export class MockDatabase {
  ...
  private static getStored<T>(key: string, defaults: T): T {
    try {
      ...
      console.warn(`Error reading localStorage for key church_cms_${key}:`, e);
      localStorage.setItem(`church_cms_${key}`, JSON.stringify(defaults));
      return defaults;
    }
  }
  ...
  static addDonation(don: Donation) {
    const items = this.getDonations();
    items.unshift(don);
    this.setStored('donations', items);
    this.addLog(
      { id: don.userId || 'guest', name: don.donorName, role: 'JEMAAT' as Role },
      'CREATE_DONATION',
      undefined,
      `Donasi Rp ${don.amount.toLocaleString('id-ID')} (${don.type})`
    );
  }

  static verifyDonation(
    id: string,
    ...
    if (index >= 0) {
      ...
      this.setStored('donations', items);
      this.addLog(actor, 'VERIFY_DONATION', JSON.stringify(before), JSON.stringify(items[index]));
    }
  }

  static deleteDonation(id: string, actor: { id: string; name: string; role: Role }) {
    const items = this.getDonations();
    const before = items.find((i) => i.id === id);
    if (!before) return;

    this.setStored('donations', items.filter((i) => i.id !== id));
    this.addLog(actor, 'DELETE_DONATION', JSON.stringify(before), undefined);
  }
  ...
  static restoreDatabase(jsonString: string, actor: { id: string; name: string; role: Role }): boolean {
    try {
      ...
      if (parsed.donations) this.setStored('donations', parsed.donations);
